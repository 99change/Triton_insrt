...
class sub_block:
    ...
    def print_block(self):
        ...
        print(f"Having empty lines at {self.empty_lines}, mma lines at {self.mma_lines}, file is {self.file}, loc is {self.loc}")
...
def builder(lines: List[str]) -> List[sub_block]:
    ...
    while i <= len(lines):
        ...
        if start_match:
            ...
            current_file = int(int(loc_match.group(1)))
            ...
            if current_sub_block.entry != ptx_line_num:
                current_sub_block.exit = ptx_line_num - 1
                sub_block_list.append(current_sub_block)
            current_sub_block = sub_block(ptx_line_num, current_file, current_loc)
